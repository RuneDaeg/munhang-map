...
export async function runExamAnalysis(file: File, onProgress: (progress: AnalysisProgress) => void, deps = dependencies) {
  ...
  if (unresolved.length) warnings.push(`${unresolved.length}개 문항(${unresolved.map(q => q.number).join(', ')}번)에 복원하지 못한 숫자·수식 문자가 남았습니다. 원문 캡처와 대조해 주세요. 성취기준 일치율은 문자 인식 정확도가 아닙니다.`);
  onProgress({ stage: 'classify', completed: finalQuestions.length, total: finalQuestions.length, percent: 100, detail: failures.length || unresolved.length || layoutReview.length ? '일부 판독 결과 확인 필요' : '분석 처리 완료' });
