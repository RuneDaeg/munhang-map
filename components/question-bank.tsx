...
import { listBankItems, loadBankSelection, type BankItem } from '@/lib/question-bank';
...
export function QuestionBank({ onClose }: { onClose: () => void }) {
  ...
  async function retrieve(ids: string[], exportMode: boolean) {
    ...
    try {
      ...
      if (exportMode) setExportQuestions(questions); else setPreview(questions[0]);
  ...
  return <>
    <Dialog open onOpenChange={(open) => { if (!open && !working && !exportQuestions) onClose(); }}>
      <DialogContent className="flex h-[90vh] flex-col sm:max-w-5xl" showCloseButton={!working && !exportQuestions}>
        ...
        <DialogDescription>이 컴퓨터에 저장한 {items.length}문항. 여러 PDF에서 문항을 골라 함께 내보낼 수 있습니다. 원문 캡처와 저장 당시의 성취기준을 그대로 사용하며 API를 다시 호출하지 않습니다.</DialogDescription>
        ...
        <div className="grid min-h-0 flex-1 gap-4 overflow-auto lg:grid-cols-[minmax(0,1fr)_300px]">
          <div className="min-w-0 space-y-2">
            ...
            {visible.map((item) => <article key={item.id} className={`rounded-xl border p-3 ${checked.has(item.id) ? 'border-primary bg-primary/5' : ''}`}>
              ...
              }} /><div className="min-w-0 flex-1"><p className="break-words text-sm font-semibold">{item.sourceFileName} · {item.number}번</p><p className="mt-1 text-sm text-primary">{item.standardCode || '미분류'} · {item.domain}</p><p className="mt-2 line-clamp-2 text-sm leading-6">{item.text}</p><Button variant="ghost" size="sm" disabled={working} onClick={() => void retrieve([item.id], false)}>원문 캡처 보기</Button></div></div>
            </article>)}
          </div>
        ...
        <div className="flex items-center justify-between gap-3 border-t pt-3"><p className="text-sm text-muted-foreground">선택한 순서로 묶습니다. 검색을 바꿔도 선택은 유지됩니다.</p><Button disabled={working || !selected.length} onClick={() => void retrieve(selected, true)}>{working ? <LoaderCircle className="animate-spin" /> : <Download />}선택 {selected.length}문항 내보내기</Button></div>
      </DialogContent>
    </Dialog>
