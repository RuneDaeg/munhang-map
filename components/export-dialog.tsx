...
export function ExportDialog({ fileName, questions, onClose }: { fileName: string; questions: AnalyzedQuestion[]; onClose: () => void }) {
  ...
  async function save(format: 'docx' | 'hwpx' | 'images') {
    ...
    try {
      ...
      if (format === 'images') await downloadCaptureArchive(fileName, questions, (completed, total) => setProgress(`캡처 이미지 묶는 중 · ${completed}/${total}장`));
  ...
  return <Dialog open onOpenChange={(open) => { if (!open && !exporting) onClose(); }}>
    <DialogContent className="max-h-[90dvh] overflow-y-auto sm:max-w-lg" showCloseButton={!exporting}>
      ...
      <section aria-labelledby="capture-export-heading" className="space-y-3 rounded-xl border bg-muted/40 p-4">
        ...
        <p className="text-sm leading-6 text-muted-foreground">문항 전체 캡처를 원본 화질 그대로 ZIP에 담습니다. 파일명에 PDF 이름·문항 번호·쪽수가 들어가며, 여러 장인 문항은 순서대로 모두 저장합니다. 텍스트·성취기준 문서는 포함하지 않으며 API를 호출하지 않습니다.</p>
        ...
        <Button variant="outline" className="w-full" disabled={exporting || !captures.imageCount || captures.missing.length > 0} onClick={() => void save('images')}><Download /> 캡처 이미지만 · ZIP</Button>
      </section>
